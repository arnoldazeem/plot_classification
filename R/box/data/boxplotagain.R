...
for(i in 1:length(list.filenames)){
  ...
  for(n in 1:ncol(numeric)) {
    ...
    if(a < ncol(numeric)){
      ...
      mypath <- file.path("/home/adaboo/Desktop/Masters/sem4/thesis/plot_classification/R/plots",paste("plotbox_", x_label, ".jpg", sep = ""))
      
      jpeg(file=mypath)
      
      #now <- plotme    
      boxplot(x_axis, xlab=x_label, ylab= y_label,horizontal=TRUE,main=title) 
