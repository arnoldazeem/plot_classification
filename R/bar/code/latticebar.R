...
for(i in 1:length(list.filenames)){
  ...
  for(n in 1:ncol(numeric)) {
    ...
    if(a < ncol(numeric)){
      ...
      mypath <- file.path("/home/adaboo/Desktop/Masters/sem4/thesis/plot_classification/R/plots",paste("leslattice_", x_label, ".png", sep = ""))
      
      filename <- paste("lessbar_sample" , x_label, sep = "" )
      
      tit = paste("Example bar lattice called ", title, " dataset", sep = "")
      #now <- plotme    
      p <-  barchart(x_axis ~ y_axis, data=numeric , main = tit, 
                  xlab = x_label, 
                  ylab = y_label)
