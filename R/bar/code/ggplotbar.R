...
for(i in 1:length(list.filenames)){
  ...
  for(n in 1:ncol(numeric)) {
    ...
    if(a < ncol(numeric)){
      ...
      mypath <- file.path("/home/adaboo/Desktop/Masters/sem4/thesis/plot_classification/R/plots",paste("easygglotsample_", x_label, ".jpg", sep = ""))
      
      filename <- paste("ggplot_barchart" , x_label, sep = "" )
      
      tit = paste("The of the next barchart is ", title, " dataset", sep = "")
      #now <- plotme    
      
      p <- ggplot2.barplot(numeric, x_label, y_label )  +
        ggtitle(tit)  +
