...
Classification involving charts has been a topic of huge interest for sometime now. This is due to the fact that further analysis of charts example: presenting to another audience is only possible after the chart is identified. 
Different techniques have over time been used for classification of charts and these techniques are be widely put into two categories; Model Based Approaches and Machine Learning Based Approaches ~\cite{amara2017convolutional}.

\section{Model Based Approaches}
In this approach handcrafted features are extracted from the charts and used to form a specific model for classifying the chart. In Zhou and Tan's ~\cite{zhou2000bar} work, features like legend, x-y-axis title, chart title and value of the bar were extracted for purposes of classifying a bar-chart. In Shao and  Futrelle's~\cite{shao2005recognition} work. Graphical elements of the charts like colors,tick marks on an axis and data point markers. Another instance where handcrafted features were used was in Inokuchi et al. work~\cite{inokuchi2000apriori}. In this work regularly appearing substructures of chart are extracted and used as features for classification. After the features are obtained, they are then represented in vector form for the classification stage. The features are stored in matrix vector with most columns containing the features and one column indicating the target of the chart. A machine learning technique is then used to recognize or classify the chart using the matrix vector as input. In Karthikeyani and Nagarajan's paper~\cite{karthikeyani2012machine} after the features were obtained SVM Classification \footnote{http://www.statsoft.com/Textbook/Support-Vector-Machines} MLP Classification
\footnote{http://www.iro.umontreal.ca/~bengioy/ift6266/H12/html/mlp\_en.html} and  KNN Classification\footnote{
https://kevinzakka.github.io/2016/07/13/k-nearest-neighbor/} were used to create a function that maps the set of features to a predefined label. This function is known as the classification model.



One technique that has outperformed the rest is CovNet. CovNets has advantage of being able to detect key points in an image without human supervision. This technique has recently achieved human-level classification performance~\cite{geirhos2017comparing}. In the rest of this chapter, three related works that our work builds on are described into details.  

\subsubsection{Machine Learning Classification Algorithms to Recognize Chart Types in Portable Document Format(PDF) Files}
Karthikeyani and Nagarajan ~\cite{karthikeyani2012machine} focuses on classifying charts found in pdf files by extracting texture features and then, using a machine learning technique for the classifying phase. Table \ref{table:pdf} shows the details of the dataset used. For the feature extraction phase, Gray Level Co-Occurrence Matrix (GLCM). GLCM is a technique which uses co-occurrence matrix to extract texture features of an image with the use of statistical equations. Using this method eleven features were extracted these were: area, median, minimum and maximum intensity, contrast, homogeneity, energy, entropy, mean, variance, standard deviation and correlation, these are mainly connected to the pixels of the image. These extracted features are then stored using a 2-dimensional matrix vector data structure. This structure has thirteen (13) columns and 'x' rows, where x is the size of the dataset. The features are stored in twelve (12) columns and the label is stored in the thirteenth column. The features and labels were stored in the below structure 
